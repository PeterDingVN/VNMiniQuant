import pandas as pd
import numpy as np
from config import DonchianCfg
...
class DonchianBreakout:
    ...
    def run(self, df_):
        ...
        df_ = df.reset_index()[['time', 'close', 'position']]

        return df_
    
if __name__ == '__main__':
    df = pd.read_parquet(r'C:\Users\HP\.0_PycharmProjects\VNMiniQuant_main\data\cached_data\stock_price_cache\VN30F1M.parquet')
    don = DonchianBreakout(config=DonchianCfg.config)
    
    out_data = don.run(df_=df)
